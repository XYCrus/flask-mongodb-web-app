...
def findhistory():
    ...
    Displays some information for the user with links to other pages.
    """
    return render_template('findhistory.html') 

@app.route('/findhistory', methods=['POST'])
def findhistory_post():
    """
    Route for POST requests to the create page.
    Accepts the form submission data for a new document and saves the document to the database.
    """
    username = request.form['fuser']
    order = -1
    if request.form['forder'] == 'a':
        order = 1

    docs = db.exampleapp.find({"name": username}).sort("created_at", order) 
